...
+ (NSAttributedString *)orderListCellWithBlackText:(NSString *)blackText grayText:(NSString *)grayText {
    
    NSString * message = [NSString stringWithFormat:@"%@%@",blackText,grayText];
    UIFont  * font = [UIFont systemFontOfSize:15];
    UIColor * blackColor = [UIColor blackColor];
    UIColor * grayColor = [UIColor grayColor];
        
    NSMutableAttributedString * attriStr = [[NSMutableAttributedString alloc] initWithString:message];
    NSDictionary * blackAttrBute = @{NSForegroundColorAttributeName:blackColor,NSFontAttributeName:font};
    NSDictionary * grayAttriBute = @{NSForegroundColorAttributeName:grayColor,NSFontAttributeName:font};
    NSRange pointBlackRange = NSMakeRange(0, blackText.length);
    [attriStr addAttributes:blackAttrBute range:pointBlackRange];
    NSRange grayBlackRange = NSMakeRange(blackText.length, grayText.length);
    [attriStr addAttributes:grayAttriBute range:grayBlackRange];
    return attriStr;
}
@end
